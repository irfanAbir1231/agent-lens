...
export function ScenarioAiResult({tier}:{tier:ScenarioTier}) { const [status,body]=byTier[tier]; return <div className="rounded-lg border p-5"><h3 className="font-bold">AI advisory status: {status}</h3><p className="mt-2 text-sm">{body}</p><div className="mt-4 flex flex-wrap gap-2"><Button href="/agents/AGENT-104" variant="outline">Open agent</Button><Button href="/alerts" variant="outline">Open live alerts</Button><Button href="/cases" variant="outline">Open live cases</Button></div></div>; }
