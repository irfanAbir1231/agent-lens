...
function mapProviderResults(agent: DataQualityDto["results"][number], generatedAt: string): DataQualityResult[] {
  return agent.provider_results.map((item) => ({
    ...
    completeness: item.component_scores.completeness,
    consistency: item.component_scores.consistency,
    sampleSize: item.component_scores.validity,
    confidenceMultiplier: item.confidence_multiplier,
    allowForecast: item.allow_forecast,
    allowAIAdvisory: item.allow_ai_advisory,
